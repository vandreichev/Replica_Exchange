...
DATA_DIRECTORY = Path(".")  # directory containing the parallel tempering data
...
trajectory_segment_length = 1  # number of snapshots in each contiguous trajectory segment
niterations = 20000  # number of iterations to use
target_temperature = 298  # target temperature for 2D free energy surface (in K)
nbins_per_torsion = 18  # number of bins per torsion dimension
...
# Create a 2D histogram for the free energy surface
plt.figure(figsize=(10, 8))

# Reshape free energy data into a grid
phi_bins = np.linspace(-180, 180, nbins_per_torsion + 1)
psi_bins = np.linspace(-180, 180, nbins_per_torsion + 1)

# Use the centers of the non-zero bins for plotting
phi_centers = [centers_nonzero[i][0] for i in range(bin_nonzero)]
psi_centers = [centers_nonzero[i][1] for i in range(bin_nonzero)]
f_centers = f_i  # free energy values at the centers

# Create a scatter plot for free energies with color mapping
plt.scatter(phi_centers, psi_centers, c=f_centers, cmap='viridis', s=100, edgecolor='k')
...
# Bar chart of counts in populated bins
plt.figure(figsize=(10, 5))
plt.bar(range(bin_nonzero), count_nonzero, tick_label=[f'({centers_nonzero[i][0]:.1f}, {centers_nonzero[i][1]:.1f})' for i in range(bin_nonzero)])
plt.title('Counts of Configurations in Free Energy Bins')
plt.xlabel('Bin (Phi, Psi)')
